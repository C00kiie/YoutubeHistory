...
opts = {
  #
  used to output less verbose information 'no_warnings': True,
  'quiet': True,
  #audios filter 'format': 'bestaudio/best',
  'postprocessors': [{
...
make long numbers easy on the eyes
...
### get statistics of a given video
def get_stats(link):
  statistics = {}
with handler_aux.YoutubeDL(opts) as ydl: #video information
for the given video
...
if h > 0:
  ...
  statistics['video_duration'] = '{}:{}'.format(m, s)# checking
if video size exceeds the normal limit[50 MB]
return statistics
...
video_arr = []# Add each result to the appropriate list, and then display the lists of #matching videos, channels, and playlists.
...
return video_arr# save a search in an html format
...
## by
default, it reades from a file called input.txt## outputs to search_term.html
for each search in the input file
if __name__ == '__main__': #for i in get_keywords():
